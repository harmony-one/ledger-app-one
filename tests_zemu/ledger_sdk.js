const { arrayify, hexlify, stripZeros, encode } = require("@harmony-js/crypto");
const { hexToNumber } = require("@harmony-js/utils");
const LEDGER_CONFIRM_REJECT = "Sign transaction rejected by the ledger";
const LEDGER_LOCKED = "Ledger wallet is locked or Harmony app is not opened.";
const CLA = 0xe0;
...
class HarmonyApp {
  ...
  async signTransaction(txn, chainId, shardId, messenger) {
    ...
    if (response.return_code === SW_ERR) throw new Error(LEDGER_LOCKED);
    else if (response.return_code !== 0x9000) {
      throw new Error(
    ...
    if (response.return_code == SW_ERR) {
      throw new Error(LEDGER_CONFIRM_REJECT);
    }
